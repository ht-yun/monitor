...
class MonitoringJob:
    ...
    async def execute(self) -> JobRunResult:
        ...
        try:
            ...
            crawler = CrawlerFactory.create_crawler(platform=self.config.platform)
            await crawler.start()

            if hasattr(crawler, "_crawled_count"):
                result.items_crawled = crawler._crawled_count
            ...
            if not items and result.items_crawled == 0:
